"""
Racing Dispatch F1 — classification prompt generator (v0.5.1).
...
1. 媒体评论、专家观点、播客观点，content_type 应为 opinion_analysis。
2. 官方公告、FIA 文件、车队官方声明，content_type 应为 official 或 procedure。
3. 技术升级、规则技术解读，content_type 应为 technical。
4. 转会、合同、队内关系、未来席位，如果没有官方确认，rumor=true 或在 risk_note_cn 说明不确定性。
5. PR 稿、重复新闻、长访谈、旧闻、roundup 应进入 excluded_articles。
6. daily_priority 要服务 RD 日报：
   - A = 必选主文
   - B = 可选主文
   - C = 备选素材
   - D = 不入选
7. 对同一事件的多篇文章必须聚合，不要重复输出多个 event_cluster。
8. 如果只有 B+ 或 B 级来源，不能把评论、猜测、传闻写成确定事实。
9. 如果来源较弱但事件重要，可以 recommended_for_daily=true，但 risk_note_cn 必须提示谨慎。
10. 输出仍然必须是严格 JSON，不要 Markdown，不要代码块，不要额外解释。
...
- content_type：内容类型（见下方可选值）
- rumor：true/false
- daily_priority：A / B / C / D
- writing_angle_cn：中文日报写作角度，例如「从法拉利队内势能变化切入，而不是写成官方队内决策。」
- risk_note_cn：编辑风险提示，例如「来源为媒体评论，不应写成官方确认。」

### content_type 可选值

news / official / analysis / opinion_analysis / rumor / interview / technical / race_result / procedure
